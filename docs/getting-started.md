...
By default `.depos.cmake` bootstraps `depos 0.5.4` into a hidden top-level `.depos/` directory next to the helper. It keeps the local registry and bootstrap state there too.

If you do not want local bootstrap, install `depos` yourself:

```bash
cargo install depos --version 0.5.4
```
